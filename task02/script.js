...
// 
    ...
    function convertMeasurements(){
        ...
        switch(convertFromSelection){
            case 'Meters':
                //test against the convert to options, due the calculations and send the result on to show results function
                if (convertToSelection == 'Feet'){
                    let feet = userNum * 3.2808;
                    showResults(feet);
                }else if (convertToSelection == 'Miles'){
                   let miles = userNum * 0.00062137;
                    showResults(miles);
                }
                break;
                //test against the convert to options, due the calculations and send the result on to show results function
             case 'Liters':
                if (convertToSelection == 'Gallons'){
                    let gallons = userNum * 0.26417; 
                    showResults(gallons);
                }else if (convertToSelection == 'Quarts'){
                    let quarts = userNum * 1.056688;
                    showResults(quarts);
                }
                break;
                //test against the convert to options, due the calculations and send the result on to show results function
            ...
            case 'Celsius':
                if (convertToSelection == 'Fahrenheit'){
                   let fahrenheit = userNum * 1.8 +32;
                    showResults(fahrenheit);
                }
                break;
        }     
    }
